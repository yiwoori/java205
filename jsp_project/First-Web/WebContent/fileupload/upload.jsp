...
<%
	...
	// 사용자가 전송한 데이터, 파일 저장 -> view 페이지로 파일의 경로 전달 -> 지정된 경로의 이미지를 출력
	...
	if(isMultipart) {
		...
		// 3. 요청 처리(form 안에 있는 input들을 분리)를 위해서 ServletFileUpload 생성
		...
		Iterator<FileItem> itr = items.iterator(); //줄 세우기
		
		while(itr.hasNext()) {
			FileItem item = itr.next();	//text, checkbox, hidden, password, file ... ->file제외(formfield):문자열/file:바이너리
			
			// file과 file이외의 폼을 구분
			if(item.isFormField()) {
				//일반 input type 처리(text, checkbox, hidden, password...)
				//파라미터 이름, 해당 폼의 데이터(Strig)
