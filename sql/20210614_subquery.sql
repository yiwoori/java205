...
--인라인 뷰 : from 절 뒤에 사용되는 부속질의, 가상 테이브처럼 사용
-- 고객번호가 2 이하인 -> custmer
-- 고객의 판매액 -> orders
...
-- 다중행 연산자 IN
--3000 이상 받는 사원이 소속된 부서와 동일한 부서에 근무하는 사원 리스트 출력
select distinct deptno
from emp
where sal>=3000;    -- 3000 이상 받는 사원이 소속된 부서 출력
select *
from emp
...
select * from customer c where 1=c.custid and c.address like '%대한민국%';
